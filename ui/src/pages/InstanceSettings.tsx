...
export function InstanceSettings() {
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("Échec de la mise à jour du heartbeat."));
    },
  });
  ...
  const disableAllMutation = useMutation({
    mutationFn: async (agentRows: InstanceSchedulerHeartbeatAgent[]) => {
      ...
      if (failures.length > 0) {
        ...
        const detail = firstError instanceof Error ? firstError.message : t("Erreur inconnue");
        throw new Error(
          failures.length === 1
            ? `${t("Échec de la désactivation d'un heartbeat :")} ${detail}`
            : `${t("Échec de la désactivation de")} ${failures.length}/${enabled.length} ${t("heartbeats. Première erreur :")} ${detail}`,
        );
      }
      return enabled;
    },
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("Échec de la désactivation de tous les heartbeats."));
    },
  });
  ...
  if (heartbeatsQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("Chargement des heartbeats...")}</div>;
  }
    ...
    return (
      <div className="text-sm text-destructive">
        {heartbeatsQuery.error instanceof Error
          ...
          : t("Échec du chargement des heartbeats.")}
      </div>
    );
  }
    ...
    <div className="max-w-5xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("Heartbeats du planificateur")}</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          {t("Agents avec un heartbeat timer activé dans toutes vos entreprises.")}
        </p>
      </div>

      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        <span><span className="font-semibold text-foreground">{activeCount}</span> {t("actifs")}</span>
        <span><span className="font-semibold text-foreground">{disabledCount}</span> {t("désactivés")}</span>
        <span><span className="font-semibold text-foreground">{grouped.length}</span> {grouped.length === 1 ? t("entreprise") : t("entreprises")}</span>
        {anyEnabled && (
          <Button
            variant="destructive"
            size="sm"
            ...
            onClick={() => {
              ...
              if (!window.confirm(`${t("Désactiver les heartbeats timer pour les")} ${enabledCount} ${noun} ${t("activés ?")}\n`)) {
                return;
              }
              disableAllMutation.mutate(agents);
            }}
          >
            {disableAllMutation.isPending ? t("Désactivation...") : t("Tout désactiver")}
          </Button>
        )}
      </div>
      ...
      {agents.length === 0 ? (
        <EmptyState
          icon={Clock3}
          message={t("Aucun heartbeat de planification ne correspond aux critères actuels.")}
        />
      ) : (
                      ...
                      >
                        ...
                        <span className="ml-auto flex items-center gap-1.5 shrink-0">
                          <Link
                            ...
                            title={t("Configuration complète de l'agent")}
                          >
                          ...
                          >
                            {saving ? "..." : agent.heartbeatEnabled ? t("Désactiver le heartbeat") : t("Activer le heartbeat")}
                          </Button>
                        </span>
                      </div>
                    );
