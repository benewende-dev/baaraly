    ...
    <div className="max-w-2xl space-y-6">
      ...
      {generalDirty && (
        <div className="flex items-center gap-2">
          ...
          >
            {generalMutation.isPending ? t("Saving changes...") : t("Save changes")}
          </Button>
...
function buildAgentSnippet(input: AgentSnippetInput) {
  ...
  const connectivityBlock =
    candidateUrls.length === 0
      ? `No candidate URLs are available. Ask your user to configure a reachable hostname in Baarali, then retry.
Suggested steps:
- choose a hostname that resolves to the Baarali host from your runtime
- run: pnpm baaraliai allowed-hostname <host>
- restart Baarali
- verify with: curl -fsS http://<host>:3100/api/health
- regenerate this invite snippet`
      : `If none are reachable, ask your user to add a reachable hostname in Baarali, restart, and retry.
Suggested command:
- pnpm baaraliai allowed-hostname <host>
Then verify with: curl -fsS <base-url>/api/health`;

  const resolutionLine = resolutionTestUrl
    ? `\nYou MUST test Baarali-to-gateway reachability, call: ${resolutionTestUrl}?url=<urlencoded-gateway-url> (using the hostname that worked above). Do not assume your 172.x is necessarily reachable from Baarali. Test it. `
    : "";

  return `You're invited to join a Baarali organization.

The URLs you should try are:
${candidateList}

Connectivity guidance:
If you are running on a different machine than Baarali, Baarali must be reachable at one of the hostnames used above.
Verify the hostname works from your runtime with: GET <base-url>/api/health

${connectivityBlock}

For your "agentName", just use your own agent name (OpenClaw or whatever your name is) and send it directly.
You also need to send a gateway URL that Baarali can access. Check your config and include that URL.${resolutionLine}
...
- Keep device auth enabled (recommended). If \`devicePrivateKeyPem\` is omitted, Baarali will generate and persist one during join so pairing approvals remain stable.
- Only use \`disableDeviceAuth=true\` for special environments where pairing cannot be completed.

Do NOT use \`/v1/responses\` or \`/hooks/*\` in this join flow.

Before you do anything, please respond to your user that you understand the instructions and you're going to work on them. Then do the step above in another session called "baarali-onboarding" and then tell your user when you're done. Update your user in intermediate steps along the way so they know what's going on.

Then after you've connected to Baarali (exchanged keys etc.) you MUST review and follow the onboarding instructions in onboarding.txt they give you.
