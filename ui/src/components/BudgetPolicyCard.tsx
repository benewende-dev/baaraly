...
function windowLabel(windowKind: BudgetPolicySummary["windowKind"]) {
  return windowKind === "lifetime" ? "Budget total" : "Budget mensuel UTC";
}
...
export function BudgetPolicyCard({
  ...
  const observedBudgetGrid = isPlain ? (
    <div className="grid gap-6 sm:grid-cols-2">
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">Observé</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">{formatCents(summary.observedAmount)}</div>
        <div className="mt-1 text-xs text-muted-foreground">
          {summary.amount > 0 ? `${summary.utilizationPercent}% de la limite` : "Aucun plafond configuré"}
        </div>
      </div>
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">Budget</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">
          {summary.amount > 0 ? formatCents(summary.amount) : "Désactivé"}
        </div>
        <div className="mt-1 text-xs text-muted-foreground">
          Alerte douce à {summary.warnPercent}%{summary.paused && summary.pauseReason ? ` · ${summary.pauseReason} pause` : ""}
        </div>
      </div>
    </div>
  ) : (
  ...
  const progressSection = (
    <div className="space-y-2">
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        <span>Restant</span>
        <span>{summary.amount > 0 ? formatCents(summary.remainingAmount) : "Illimité"}</span>
      </div>
    ...
    <div className="flex items-start gap-2 rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-100">
      ...
      <div>
        {summary.scopeType === "project"
          ? "L'exécution est en pause pour ce projet jusqu'à ce que le budget soit augmenté ou que l'incident soit rejeté."
          : "Les exécutions sont en pause pour cet agent jusqu'à ce que le budget soit augmenté ou que l'incident soit rejeté."}
      </div>
    </div>
  ) : null;
