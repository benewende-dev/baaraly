...
type BaaralySprite = (typeof BAARALY_SPRITES)[number];
...
function spriteSize(sprite: BaaralySprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = BAARALY_SPRITES[(baseCol + baseRow) % BAARALY_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = BAARALY_SPRITES[Math.floor(Math.random() * BAARALY_SPRITES.length)]!;
