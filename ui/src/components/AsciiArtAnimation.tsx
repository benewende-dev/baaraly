...
type BaaraliSprite = (typeof BAARALI_SPRITES)[number];
...
function spriteSize(sprite: BaaraliSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = BAARALI_SPRITES[(baseCol + baseRow) % BAARALI_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = BAARALI_SPRITES[Math.floor(Math.random() * BAARALI_SPRITES.length)]!;
