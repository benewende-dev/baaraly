...
import { LiveRunStepper } from "./LiveRunStepper";
          ...
          return (
            <section key={run.id} className="px-4 py-4">
                ...
                <div className="flex items-center gap-2">
                  {isActive && (
                    ...
                    >
                      ...
                      {cancellingRunIds.has(run.id) ? "Arrêt…" : "Arrêter"}
                    </button>
                  )}
                  <Link
              ...
              <div className="max-h-[320px] overflow-y-auto pr-1">
                {isMobile ? (
                  /* Mobile : stepper simplifié en français */
                  <LiveRunStepper
                    entries={transcript}
                    streaming={isActive}
                  />
                ) : (
                  /* Desktop : transcript technique complet */
                  <RunTranscriptView
                    entries={transcript}
                    density="compact"
                    limit={8}
                    streaming={isActive}
                    collapseStdout
                    emptyMessage={hasOutputForRun(run.id) ? "En attente de l'analyse..." : "En attente de la sortie..."}
                  />
                )}
              </div>
            </section>
