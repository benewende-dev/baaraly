 ...
 * `BAARALI_ALLOWED_ATTACHMENT_TYPES` environment variable to a
 * comma-separated list of MIME types or wildcard patterns to expand the
 * allowed set.
 *
 * Examples:
 *   BAARALI_ALLOWED_ATTACHMENT_TYPES=image/*,application/pdf
 *   BAARALI_ALLOWED_ATTACHMENT_TYPES=image/*,application/pdf,text/*
 *
...
const allowedPatterns: string[] = parseAllowedTypes(
  process.env.BAARALI_ALLOWED_ATTACHMENT_TYPES,
);
...
export const MAX_ATTACHMENT_BYTES =
  Number(process.env.BAARALI_ATTACHMENT_MAX_BYTES) || 10 * 1024 * 1024;
