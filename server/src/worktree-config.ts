...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    ...
    "# Generated by Baarali worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.BAARALI_IN_WORKTREE !== "true") return null;

  const configPath = resolvePaperclipConfigPath(overrideConfigPath);
  const envPath = resolveBaaraliEnvPath(configPath);
  const worktreeRoot = path.resolve(path.dirname(configPath), "..");
  const worktreeName = nonEmpty(env.BAARALI_WORKTREE_NAME) ?? path.basename(worktreeRoot);
  const instanceId = nonEmpty(env.BAARALI_INSTANCE_ID) ?? sanitizeWorktreeInstanceId(worktreeName);
  const homeDir = resolveHomeAwarePath(
    nonEmpty(env.BAARALI_HOME) ??
      nonEmpty(env.BAARALI_WORKTREES_DIR) ??
      "~/.baarali-worktrees",
  );
...
function resolveRepoManagedWorktreesRoot(worktreeRoot: string): string | null {
  ...
  const marker = `${path.sep}.baarali${path.sep}worktrees${path.sep}`;
  const index = normalized.indexOf(marker);
  if (index === -1) return null;
  const repoRoot = normalized.slice(0, index);
  return path.resolve(repoRoot, ".baarali", "worktrees");
}
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".baarali", "config.json");
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.BAARALI_HOME = context.homeDir;
  process.env.BAARALI_INSTANCE_ID = context.instanceId;
  process.env.BAARALI_CONFIG = context.configPath;
  process.env.BAARALI_CONTEXT = context.contextPath;
  process.env.BAARALI_WORKTREE_NAME = context.worktreeName;
  ...
  const desiredEnvEntries: Record<string, string> = {
    ...existingEnvEntries,
    BAARALI_HOME: context.homeDir,
    BAARALI_INSTANCE_ID: context.instanceId,
    BAARALI_CONFIG: context.configPath,
    BAARALI_CONTEXT: context.contextPath,
    BAARALI_IN_WORKTREE: "true",
    BAARALI_WORKTREE_NAME: context.worktreeName,
  };
