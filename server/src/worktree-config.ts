...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    ...
    "# Generated by Baaraly worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.BAARALY_IN_WORKTREE !== "true") return null;

  const configPath = resolvePaperclipConfigPath(overrideConfigPath);
  const envPath = resolveBaaralyEnvPath(configPath);
  const worktreeRoot = path.resolve(path.dirname(configPath), "..");
  const worktreeName = nonEmpty(env.BAARALY_WORKTREE_NAME) ?? path.basename(worktreeRoot);
  const instanceId = nonEmpty(env.BAARALY_INSTANCE_ID) ?? sanitizeWorktreeInstanceId(worktreeName);
  const homeDir = resolveHomeAwarePath(
    nonEmpty(env.BAARALY_HOME) ??
      nonEmpty(env.BAARALY_WORKTREES_DIR) ??
      "~/.baaraly-worktrees",
  );
...
function resolveRepoManagedWorktreesRoot(worktreeRoot: string): string | null {
  ...
  const marker = `${path.sep}.baaraly${path.sep}worktrees${path.sep}`;
  const index = normalized.indexOf(marker);
  if (index === -1) return null;
  const repoRoot = normalized.slice(0, index);
  return path.resolve(repoRoot, ".baaraly", "worktrees");
}
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".baaraly", "config.json");
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.BAARALY_HOME = context.homeDir;
  process.env.BAARALY_INSTANCE_ID = context.instanceId;
  process.env.BAARALY_CONFIG = context.configPath;
  process.env.BAARALY_CONTEXT = context.contextPath;
  process.env.BAARALY_WORKTREE_NAME = context.worktreeName;
  ...
  const desiredEnvEntries: Record<string, string> = {
    ...existingEnvEntries,
    BAARALY_HOME: context.homeDir,
    BAARALY_INSTANCE_ID: context.instanceId,
    BAARALY_CONFIG: context.configPath,
    BAARALY_CONTEXT: context.contextPath,
    BAARALY_IN_WORKTREE: "true",
    BAARALY_WORKTREE_NAME: context.worktreeName,
  };
