...
describe("company portability", () => {
    ...
    const companySkills = [
      {
        ...
        slug: "paperclip",
        name: "paperclip",
        description: "Baaraly coordination skill",
        markdown: "---\nname: baaraly\ndescription: Baaraly coordination skill\n---\n\n# Baaraly\n",
        sourceType: "github",
        sourceLocator: "https://github.com/baaralyai/baaraly/tree/master/skills/baaraly",
  ...
  it("parses canonical GitHub import URLs with explicit ref and package path", () => {
    expect(
      parseGitHubSourceUrl("https://github.com/baaralyai/companies?ref=feature%2Fdemo&path=gstack"),
    ).toEqual({
      owner: "baaralyai",
      repo: "companies",
      ref: "feature/demo",
  ...
  it("parses canonical GitHub import URLs with explicit companyPath", () => {
    expect(
      parseGitHubSourceUrl(
        "https://github.com/baaralyai/companies?ref=abc123&companyPath=gstack%2FCOMPANY.md",
      ),
    ).toEqual({
      owner: "baaralyai",
      repo: "companies",
  ...
  it("exports referenced skills as stubs by default with sanitized Baaraly extension data", async () => {
    ...
    expect(asTextFile(exported.files["COMPANY.md"])).toContain('name: "Baaraly"');
    ...
    expect(asTextFile(exported.files["agents/claudecoder/AGENTS.md"])).toContain(`- "${baaralyKey}"`);
    expect(asTextFile(exported.files["agents/cmo/AGENTS.md"])).not.toContain("skills:");
    expect(asTextFile(exported.files["skills/baaralyai/baaraly/baaraly/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/baaralyai/baaraly/baaraly/SKILL.md"])).toContain('kind: "github-dir"');
    expect(exported.files["skills/baaralyai/baaraly/baaraly/references/api.md"]).toBeUndefined();
    ...
    const extension = asTextFile(exported.files[".baaraly.yaml"]);
    expect(extension).toContain('schema: "baaraly/v1"');
    ...
    expect(extension).not.toContain("baaralySkillSync");
  ...
  it("exports default sidebar order into the Baaraly extension and manifest", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(asTextFile(exported.files[".baaraly.yaml"])).toContain([
      "sidebar:",
      "  agents:",
      '    - "claudecoder"',
      '    - "cmo"',
  ...
  it("expands referenced skills when requested", async () => {
    ...
    expect(asTextFile(exported.files["skills/baaralyai/baaraly/baaraly/SKILL.md"])).toContain("# Baaraly");
    expect(asTextFile(exported.files["skills/baaralyai/baaraly/baaraly/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/baaralyai/baaraly/baaraly/references/api.md"])).toContain("# API");
  });
  ...
  it("exports only selected skills when skills filter is provided", async () => {
    ...
    expect(asTextFile(exported.files["skills/company/PAP/company-playbook/SKILL.md"])).toContain("# Company Playbook");
    expect(exported.files["skills/baaralyai/baaraly/baaraly/SKILL.md"]).toBeUndefined();
  });
  ...
  it("warns and exports all skills when skills filter matches nothing", async () => {
    ...
    expect(exported.files["skills/company/PAP/company-playbook/SKILL.md"]).toBeDefined();
    expect(exported.files["skills/baaralyai/baaraly/baaraly/SKILL.md"]).toBeDefined();
  });

  it("exports the company logo into images/ and references it from .baaraly.yaml", async () => {
    const storage = {
    ...
    expect(exported.files[".baaraly.yaml"]).toContain('logoPath: "images/company-logo.png"');
  });
  ...
  it("exports duplicate skill slugs into readable namespaced paths", async () => {
    ...
    companySkillSvc.listFull.mockResolvedValue([
      ...
      },
      {
        id: "skill-baaraly",
        companyId: "company-1",
        key: "baaralyai/baaraly/release-changelog",
        slug: "release-changelog",
        name: "release-changelog",
        description: "Bundled release changelog skill",
        markdown: "---\nname: release-changelog\n---\n\n# Bundled Release Changelog\n",
        sourceType: "github",
        sourceLocator: "https://github.com/baaralyai/baaraly/tree/master/skills/release-changelog",
        sourceRef: "0123456789abcdef0123456789abcdef01234567",
        trustLevel: "markdown_only",
        compatibility: "compatible",
        fileInventory: [{ path: "SKILL.md", kind: "skill" }],
    ...
    expect(asTextFile(exported.files["skills/baaralyai/baaraly/release-changelog/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/baaralyai/baaraly/release-changelog/SKILL.md"])).toContain("baaralyai/baaraly/release-changelog");
  });
  ...
  it("exports portable project workspace metadata and remaps it on import", async () => {
        ...
        workspaces: [
          {
            ...
            name: "Main Repo",
            sourceType: "git_repo",
            cwd: "/Users/dotta/baaraly",
            repoUrl: "https://github.com/baaralyai/baaraly.git",
            repoRef: "main",
            defaultRef: "main",
            visibility: "default",
            setupCommand: "pnpm install",
            cleanupCommand: "rm -rf .baaraly-tmp",
            remoteProvider: null,
    ...
    expect(extension).toContain('repoUrl: "https://github.com/baaralyai/baaraly.git"');
    expect(extension).toContain('defaultProjectWorkspaceKey: "main-repo"');
    expect(extension).toContain('projectWorkspaceKey: "main-repo"');
    expect(extension).not.toContain("/Users/dotta/baaraly");
    ...
    await portability.importBundle({
      ...
      target: {
        mode: "new_company",
        newCompanyName: "Imported Baaraly",
      },
  ...
  it("infers portable git metadata from a local checkout without task warning fan-out", async () => {
    const portability = companyPortabilityService({} as any);
    const repoDir = await fs.mkdtemp(path.join(os.tmpdir(), "baaraly-portability-git-"));
    execFileSync("git", ["init"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["checkout", "-b", "main"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["remote", "add", "origin", "https://github.com/baaralyai/baaraly.git"], {
      cwd: repoDir,
      stdio: "ignore",
    });
  ...
  it("reads env inputs back from .baaraly.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("exports routines as recurring task packages with Baaraly routine extensions", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("imports recurring task packages as routines instead of one-time issues", async () => {
    ...
    const preview = await portability.previewImport({
      source: { type: "inline", rootPath: "baaraly-demo", files },
      include: { company: true, agents: true, projects: true, issues: true, skills: false },
      target: { mode: "new_company", newCompanyName: "Imported Baaraly" },
      agents: "all",
      collisionStrategy: "rename",
    });
  ...
  it("migrates legacy schedule.recurrence imports into routine triggers", async () => {
    ...
    const files = {
      "COMPANY.md": ['---', 'schema: "agentcompanies/v1"', 'name: "Imported Baaraly"', "---", ""].join("\n"),
  ...
  it("imports a vendor-neutral package without .baaraly.yaml", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(preview.errors).toEqual([]);
    expect(preview.manifest.company?.name).toBe("Imported Baaraly");
  ...
  it("imports a packaged company logo and attaches it to the target company", async () => {
    ...
    exported.files[".baaraly.yaml"] = `${exported.files[".baaraly.yaml"]}`.replace(
      'brandColor: "#5c5fff"\n',
