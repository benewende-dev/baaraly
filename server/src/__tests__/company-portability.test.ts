...
describe("company portability", () => {
    ...
    const companySkills = [
      {
        ...
        slug: "paperclip",
        name: "paperclip",
        description: "Baarali coordination skill",
        markdown: "---\nname: baarali\ndescription: Baarali coordination skill\n---\n\n# Baarali\n",
        sourceType: "github",
        sourceLocator: "https://github.com/baaraliai/baarali/tree/master/skills/baarali",
  ...
  it("parses canonical GitHub import URLs with explicit ref and package path", () => {
    expect(
      parseGitHubSourceUrl("https://github.com/baaraliai/companies?ref=feature%2Fdemo&path=gstack"),
    ).toEqual({
      owner: "baaraliai",
      repo: "companies",
      ref: "feature/demo",
  ...
  it("parses canonical GitHub import URLs with explicit companyPath", () => {
    expect(
      parseGitHubSourceUrl(
        "https://github.com/baaraliai/companies?ref=abc123&companyPath=gstack%2FCOMPANY.md",
      ),
    ).toEqual({
      owner: "baaraliai",
      repo: "companies",
  ...
  it("exports referenced skills as stubs by default with sanitized Baarali extension data", async () => {
    ...
    expect(asTextFile(exported.files["COMPANY.md"])).toContain('name: "Baarali"');
    ...
    expect(asTextFile(exported.files["agents/claudecoder/AGENTS.md"])).toContain(`- "${baaraliKey}"`);
    expect(asTextFile(exported.files["agents/cmo/AGENTS.md"])).not.toContain("skills:");
    expect(asTextFile(exported.files["skills/baaraliai/baarali/baarali/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/baaraliai/baarali/baarali/SKILL.md"])).toContain('kind: "github-dir"');
    expect(exported.files["skills/baaraliai/baarali/baarali/references/api.md"]).toBeUndefined();
    ...
    const extension = asTextFile(exported.files[".baarali.yaml"]);
    expect(extension).toContain('schema: "baarali/v1"');
    ...
    expect(extension).not.toContain("baaraliSkillSync");
  ...
  it("exports default sidebar order into the Baarali extension and manifest", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(asTextFile(exported.files[".baarali.yaml"])).toContain([
      "sidebar:",
      "  agents:",
      '    - "claudecoder"',
      '    - "cmo"',
  ...
  it("expands referenced skills when requested", async () => {
    ...
    expect(asTextFile(exported.files["skills/baaraliai/baarali/baarali/SKILL.md"])).toContain("# Baarali");
    expect(asTextFile(exported.files["skills/baaraliai/baarali/baarali/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/baaraliai/baarali/baarali/references/api.md"])).toContain("# API");
  });
  ...
  it("exports only selected skills when skills filter is provided", async () => {
    ...
    expect(asTextFile(exported.files["skills/company/PAP/company-playbook/SKILL.md"])).toContain("# Company Playbook");
    expect(exported.files["skills/baaraliai/baarali/baarali/SKILL.md"]).toBeUndefined();
  });
  ...
  it("warns and exports all skills when skills filter matches nothing", async () => {
    ...
    expect(exported.files["skills/company/PAP/company-playbook/SKILL.md"]).toBeDefined();
    expect(exported.files["skills/baaraliai/baarali/baarali/SKILL.md"]).toBeDefined();
  });

  it("exports the company logo into images/ and references it from .baarali.yaml", async () => {
    const storage = {
    ...
    expect(exported.files[".baarali.yaml"]).toContain('logoPath: "images/company-logo.png"');
  });
  ...
  it("exports duplicate skill slugs into readable namespaced paths", async () => {
    ...
    companySkillSvc.listFull.mockResolvedValue([
      ...
      },
      {
        id: "skill-baarali",
        companyId: "company-1",
        key: "baaraliai/baarali/release-changelog",
        slug: "release-changelog",
        name: "release-changelog",
        description: "Bundled release changelog skill",
        markdown: "---\nname: release-changelog\n---\n\n# Bundled Release Changelog\n",
        sourceType: "github",
        sourceLocator: "https://github.com/baaraliai/baarali/tree/master/skills/release-changelog",
        sourceRef: "0123456789abcdef0123456789abcdef01234567",
        trustLevel: "markdown_only",
        compatibility: "compatible",
        fileInventory: [{ path: "SKILL.md", kind: "skill" }],
    ...
    expect(asTextFile(exported.files["skills/baaraliai/baarali/release-changelog/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/baaraliai/baarali/release-changelog/SKILL.md"])).toContain("baaraliai/baarali/release-changelog");
  });
  ...
  it("exports portable project workspace metadata and remaps it on import", async () => {
        ...
        workspaces: [
          {
            ...
            name: "Main Repo",
            sourceType: "git_repo",
            cwd: "/Users/dotta/baarali",
            repoUrl: "https://github.com/baaraliai/baarali.git",
            repoRef: "main",
            defaultRef: "main",
            visibility: "default",
            setupCommand: "pnpm install",
            cleanupCommand: "rm -rf .baarali-tmp",
            remoteProvider: null,
    ...
    expect(extension).toContain('repoUrl: "https://github.com/baaraliai/baarali.git"');
    expect(extension).toContain('defaultProjectWorkspaceKey: "main-repo"');
    expect(extension).toContain('projectWorkspaceKey: "main-repo"');
    expect(extension).not.toContain("/Users/dotta/baarali");
    ...
    await portability.importBundle({
      ...
      target: {
        mode: "new_company",
        newCompanyName: "Imported Baarali",
      },
  ...
  it("infers portable git metadata from a local checkout without task warning fan-out", async () => {
    const portability = companyPortabilityService({} as any);
    const repoDir = await fs.mkdtemp(path.join(os.tmpdir(), "baarali-portability-git-"));
    execFileSync("git", ["init"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["checkout", "-b", "main"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["remote", "add", "origin", "https://github.com/baaraliai/baarali.git"], {
      cwd: repoDir,
      stdio: "ignore",
    });
  ...
  it("reads env inputs back from .baarali.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("exports routines as recurring task packages with Baarali routine extensions", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("imports recurring task packages as routines instead of one-time issues", async () => {
    ...
    const preview = await portability.previewImport({
      source: { type: "inline", rootPath: "baarali-demo", files },
      include: { company: true, agents: true, projects: true, issues: true, skills: false },
      target: { mode: "new_company", newCompanyName: "Imported Baarali" },
      agents: "all",
      collisionStrategy: "rename",
    });
  ...
  it("migrates legacy schedule.recurrence imports into routine triggers", async () => {
    ...
    const files = {
      "COMPANY.md": ['---', 'schema: "agentcompanies/v1"', 'name: "Imported Baarali"', "---", ""].join("\n"),
  ...
  it("imports a vendor-neutral package without .baarali.yaml", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(preview.errors).toEqual([]);
    expect(preview.manifest.company?.name).toBe("Imported Baarali");
  ...
  it("imports a packaged company logo and attaches it to the target company", async () => {
    ...
    exported.files[".baarali.yaml"] = `${exported.files[".baarali.yaml"]}`.replace(
      'brandColor: "#5c5fff"\n',
