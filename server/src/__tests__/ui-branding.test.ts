...
<head>
    <!-- BAARALI_RUNTIME_BRANDING_START -->
    <!-- BAARALI_RUNTIME_BRANDING_END -->
    <!-- BAARALI_FAVICON_START -->
...
describe("ui branding", () => {
  it("detects worktree mode from BAARALI_IN_WORKTREE", () => {
    expect(isWorktreeUiBrandingEnabled({ BAARALI_IN_WORKTREE: "true" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ BAARALI_IN_WORKTREE: "1" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ BAARALI_IN_WORKTREE: "false" })).toBe(false);
  });

  it("resolves name, color, and text color for worktree branding", () => {
    const branding = getWorktreeUiBranding({
      BAARALI_IN_WORKTREE: "true",
      BAARALI_WORKTREE_NAME: "baarali-pr-432",
      BAARALI_WORKTREE_COLOR: "#4f86f7",
    });

    expect(branding.enabled).toBe(true);
    expect(branding.name).toBe("baarali-pr-432");
  ...
  it("renders runtime branding metadata for the ui", () => {
    ...
    expect(meta).toContain('name="baarali-worktree-name"');
    expect(meta).toContain('content="baarali-pr-432"');
    expect(meta).toContain('name="baarali-worktree-color"');
  });
  ...
  it("rewrites the favicon and runtime branding blocks for worktree instances only", () => {
    ...
    expect(branded).toContain('name="baarali-worktree-name"');
    ...
    expect(defaultHtml).not.toContain('name="baarali-worktree-name"');
  });
});
