...
<head>
    <!-- BAARALY_RUNTIME_BRANDING_START -->
    <!-- BAARALY_RUNTIME_BRANDING_END -->
    <!-- BAARALY_FAVICON_START -->
...
describe("ui branding", () => {
  it("detects worktree mode from BAARALY_IN_WORKTREE", () => {
    expect(isWorktreeUiBrandingEnabled({ BAARALY_IN_WORKTREE: "true" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ BAARALY_IN_WORKTREE: "1" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ BAARALY_IN_WORKTREE: "false" })).toBe(false);
  });

  it("resolves name, color, and text color for worktree branding", () => {
    const branding = getWorktreeUiBranding({
      BAARALY_IN_WORKTREE: "true",
      BAARALY_WORKTREE_NAME: "baaraly-pr-432",
      BAARALY_WORKTREE_COLOR: "#4f86f7",
    });

    expect(branding.enabled).toBe(true);
    expect(branding.name).toBe("baaraly-pr-432");
  ...
  it("renders runtime branding metadata for the ui", () => {
    ...
    expect(meta).toContain('name="baaraly-worktree-name"');
    expect(meta).toContain('content="baaraly-pr-432"');
    expect(meta).toContain('name="baaraly-worktree-color"');
  });
  ...
  it("rewrites the favicon and runtime branding blocks for worktree instances only", () => {
    ...
    expect(branded).toContain('name="baaraly-worktree-name"');
    ...
    expect(defaultHtml).not.toContain('name="baaraly-worktree-name"');
  });
});
