...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("Baaraly OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("Suggested Baaraly base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("baaralyApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.baaralyApiUrl");
    expect(text).toContain("~/.openclaw/workspace/baaraly-claimed-api-key.json");
    expect(text).toContain("BAARALY_API_KEY");
