...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("Baarali OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("Suggested Baarali base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("baaraliApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.baaraliApiUrl");
    expect(text).toContain("~/.openclaw/workspace/baarali-claimed-api-key.json");
    expect(text).toContain("BAARALI_API_KEY");
